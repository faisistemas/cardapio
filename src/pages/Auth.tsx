...
const Auth = () => {
  ...
  const checkRolesAndRedirect = async (userId: string) => {
    try {
      ...
      setIsLoading(false);
      toast({ title: 'Acesso negado', description: 'Sua conta não possui permissão.', variant: 'destructive' });
      await supabase.auth.signOut();
    } catch (error) {
      console.error('Error checking roles:', error);
      setIsLoading(false);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (session) await checkRolesAndRedirect(session.user.id);
    };
    checkAuth();

    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (session && event === 'SIGNED_IN') {
        setTimeout(() => checkRolesAndRedirect(session.user.id), 0);
      }
    });
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirectUrl = slug ? `${window.location.origin}/r/${slug}/auth` : `${window.location.origin}/auth`;
      const { error } = await supabase.auth.resetPasswordForEmail(formData.email, { redirectTo: redirectUrl });
      if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) await checkRolesAndRedirect(data.user.id);
    } catch {
      toast({ title: 'Erro inesperado', description: 'Tente novamente mais tarde', variant: 'destructive' });
      setIsLoading(false);
    }
  ...
  const getTitle = () => mode === 'login' ? 'Entrar no Painel' : mode === 'signup' ? 'Criar Conta' : 'Recuperar Senha';
  const getSubtitle = () => mode === 'login' ? 'Acesse sua conta' : mode === 'signup' ? 'Preencha os dados' : 'Digite seu email';
  const getButtonText = () => mode === 'login' ? 'Entrar' : mode === 'signup' ? 'Criar Conta' : 'Enviar Link';

  return (
    <>
      <Helmet><title>{getTitle()} - Painel Admin</title></Helmet>
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-md">
          <div className="flex justify-center mb-8">
            <div className="h-20 w-20 rounded-full bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center shadow-lg">
          ...
          <div className="bg-card rounded-3xl shadow-card p-8">
            {(mode === 'forgot' || mode === 'signup') && (
              <button type="button" onClick={() => setMode('login')} className="flex items-center gap-1 text-sm mb-4">
                <ArrowLeft className="w-4 h-4" /> Voltar ao login
              </button>
            )}
            <h1 className="text-2xl font-bold text-center mb-2">{getTitle()}</h1>
            <p className="text-center text-muted-foreground mb-8">{getSubtitle()}</p>
                        <form onSubmit={handleSubmit} className="space-y-4">
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                <Input
