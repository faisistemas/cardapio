...
import { useTheme } from '@/hooks/useTheme';
import { supabase } from '@/integrations/supabase/client';
import { useRestaurantIdBySlug } from '@/hooks/useOrders';

export default function Kitchen() {
  const navigate = useNavigate();
  const { slug } = useParams<{ slug: string }>();

  // restaurantId correto pelo slug
  const restaurantId = useRestaurantIdBySlug();
  ...
  // hook da cozinha usando restaurantId
  const { items, isLoading } = useKitchenItems(undefined, restaurantId);
