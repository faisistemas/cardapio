...
import { Badge } from '@/components/ui/badge';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Loader2, Plus, Pencil, Trash2, Package, Star, X } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useSubscriptionPlans, useCreatePlan, useUpdatePlan, useDeletePlan } from '@/hooks/useSubscriptionPlans';
import type { SubscriptionPlan } from '@/types/reseller';
import {
...
export function SubscriptionPlansManager() {
  ...
  const handleOpenEdit = (plan: SubscriptionPlan) => {
    ...
    setFormData({
      ...
      sort_order: plan.sort_order,
      features: plan.features || [],
      trial_days: plan.trial_days ?? 0, // <-- ADICIONADO
    });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const planData = {
        ...
        description: formData.description.trim() || null,
        monthly_fee: parseFloat(formData.monthly_fee) || 0,
        trial_days: formData.trial_days, // <-- CORRIGIDO
        is_active: formData.is_active,
        is_popular: formData.is_popular,
        sort_order: formData.sort_order,
        features: formData.features.length > 0 ? formData.features : null,
