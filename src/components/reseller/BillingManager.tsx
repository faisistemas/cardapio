...
export function BillingManager({ restaurant, resellerId, mpIntegrationEnabled }: BillingManagerProps) {
  ...
  // 🔥 ESTADO PARA SALVAR O LINK RETORNADO
  const [paymentLink, setPaymentLink] = useState<string | null>(null);

  const updateRestaurant = useUpdateRestaurant();

  // 🔥 AJUSTE DO HOOK PARA RECEBER O LINK
  const createSubscription = useCreateMPSubscription(async (link: string) => {
    setPaymentLink(link);

    // 🔥 SALVA O LINK NO RESTAURANTE
    await updateRestaurant.mutateAsync({
      id: restaurant.id,
      mp_init_point: link,
      mp_payer_email: payerEmail.trim(),
    });
  });
  ...
  const handleCopyLink = async () => {
    const link = restaurant.mp_init_point || paymentLink;
    if (!link) return;
      ...
      <Card>
        ...
        <CardContent>
          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              O Mercado Pago não está configurado. Configure em <strong>Configurações → Mercado Pago</strong>.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Link de pagamento existente */}
          {(restaurant.mp_init_point || paymentLink) && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label>Link de Pagamento</Label>
                {restaurant.mp_subscription_status === 'pending' && (
              ...
              <div className="flex gap-2">
                <Input 
                  value={restaurant.mp_init_point || paymentLink || ''} 
                  readOnly 
                  className="text-xs font-mono"
                />
                <Button variant="outline" size="icon" onClick={handleCopyLink} title="Copiar">
                ...
                <Button 
                  variant="outline" 
                  size="icon"
                  onClick={() => window.open(restaurant.mp_init_point || paymentLink!, '_blank')}
                  title="Abrir link"
                >
                  <ExternalLink className="h-4 w-4" />
                </Button>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <WhatsAppButton
                  ...
                  ownerName={restaurant.owner_name || null}
                  restaurantName={restaurant.name}
                  paymentLink={restaurant.mp_init_point || paymentLink!}
                  monthlyFee={restaurant.monthly_fee}
                  className="w-full"
                  variant="default"
                />
